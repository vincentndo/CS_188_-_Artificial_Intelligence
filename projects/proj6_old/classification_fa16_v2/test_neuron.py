...
sn = neuron.SigmoidNeuron(len(featurized_training_data[0]))
...
for i in range(num_times_to_train):
	sn.train(featurized_training_data, alpha, training_labels)

training_accuracy = dcu.zero_one_loss(sn, featurized_training_data, training_labels) * 100
test_accuracy = dcu.zero_one_loss(sn, featurized_test_data, test_labels) * 100
