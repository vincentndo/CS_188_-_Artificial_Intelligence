...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # print(eliminationOrder, currentFactorsList)
        ...
        for eliminateVariable in eliminationOrder:
            ...
            # fullJoint = joinFactors(currentFactorsList)
            # print(fullJoint)
            if len(joinedFactor.unconditionedVariables()) != 1 or eliminateVariable not in joinedFactor.unconditionedVariables():
                eliminatedFactor = eliminate(joinedFactor, eliminateVariable)
                # normalizedFactor = normalize(eliminatedFactor)
