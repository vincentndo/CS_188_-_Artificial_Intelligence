...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return [ self.output_map[p] for p in node.get_parents() ]
    ...
    def add(self, node):
        ...
        self.output_map[node] = output
        self.gradient_map[node] = np.zeros_like(output)
    ...
    def backprop(self):
        ...
        for node in self.get_nodes()[::-1]:
            ...
            gradients = node.backward(self.get_inputs(node), self.gradient_map[node])
            for p, g in zip(parents, gradients):
                self.gradient_map[p] += g
    ...
    def step(self, step_size):
        ...
        for n in nodes:
            if isinstance(n, Variable):
                n.data -= step_size * self.get_gradient(n)
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient, np.ones(inputs[0].shape[0]).dot(gradient)]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        n = inputs[0].size
        return [(gradient * inputs[0] - gradient * inputs[1]) / n, (gradient * inputs[1] - gradient * inputs[0]) / n]
