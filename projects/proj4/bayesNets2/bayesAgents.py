...
def constructBayesNet(gameState):
    ...
    edges += [ (X_POS_VAR, HOUSE_VARS[0]), (X_POS_VAR, HOUSE_VARS[1]), (Y_POS_VAR, HOUSE_VARS[0]), (Y_POS_VAR, HOUSE_VARS[1]) ]

    edgesFoodToObservations = [ (HOUSE_VARS[0], obsVar) for obsVar in obsVars ]
    edgesGhostToObservations = [ (HOUSE_VARS[1], obsVar) for obsVar in obsVars ]

    edges += edgesFoodToObservations + edgesGhostToObservations
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        ...
        if housePos == bottomLeftPos:
            houseCenter = BOTTOM_LEFT_VAL
        elif housePos == topLeftPos:
            houseCenter = TOP_LEFT_VAL
        elif housePos == bottomRightPos:
            houseCenter = BOTTOM_RIGHT_VAL
        elif housePos == topRightPos:
            houseCenter = TOP_RIGHT_VAL
        ...
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                prob = 0
                if foodHouse != houseCenter:
                    if ghostHouse != houseCenter:
                        if assignment[obsVar] == 'none':
                            prob = 1
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        ...
        if factor.getProbability(assignment) > max_val:
            ...
            max_val = factor.getProbability(assignment)
            loc = assignment[FOOD_HOUSE_VAR]
    
    evidence[FOOD_HOUSE_VAR] = loc
    return evidence
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            ...
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftProb = factor.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightProb = factor.getProbability(assignment)

        leftExpectedValue = WON_GAME_REWARD * leftProb + GHOST_COLLISION_REWARD * rightProb
        rightExpectedValue = WON_GAME_REWARD * rightProb + GHOST_COLLISION_REWARD * leftProb
