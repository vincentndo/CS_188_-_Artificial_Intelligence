...
def simple_image_featurization(image):
    ...
    vector = np.zeros( (w*h), "float" )

    for i in range(w):
        for j in range(h):
            vector[i * w + j] = image.getPixel(j, i)
...
def zero_one_loss(classifier, samples, labels):
    ...
    n = len(labels)
    return sum( [zero_one_loss_ss(classifier, samples[i], labels[i]) for i in range(n)] ) / n
...
def convert_weight_vector_to_matrix(weight_vector, w, h, bias):
    ...
    return weight_vector.reshape(w, h)
...
def convert_perceptron_weights_to_2D_array_with_ten_columns(p):
    ...
    p.weight_vectors = p.weight_vectors.transpose()
