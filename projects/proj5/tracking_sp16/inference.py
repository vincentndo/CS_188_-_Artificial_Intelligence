...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        normalizing_factor = self.total()

        if normalizing_factor != 0.0:
            for i in self:
                self[i] /= normalizing_factor
    ...
    def sample(self):
        ...
        for i in range(1, len(item_list)):
            item_i = (item_list[i][0], item_list[i][1] + item_list[i-1][1])
            item_list[i] = item_i

        pick = random.random() * total_values
        index = 0

        while pick >= item_list[index][1]:
            index += 1

        ret = item_list[index][0]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                w *= self.getObservationProb(observation[i], pacmanPosition, p[i], jailPosition)
            weights[p] += w
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                positions.append(newPosDist.sample())

            newParticle = positions
