...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.layer_num = 100
        self.W = [nn.Variable(1, self.layer_num), nn.Variable(self.layer_num, 1)]
        self.b = [nn.Variable(1, self.layer_num), nn.Variable(1, 1)]
    ...
    def run(self, x, y=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        graph = nn.Graph( [self.W[0], self.b[0], self.W[1], self.b[1]] )

        input_x = nn.Input(graph, x)
        matrix_multiplier_1 = nn.MatrixMultiply(graph, input_x, self.W[0])
        matrix_adder_1 = nn.MatrixVectorAdd(graph, matrix_multiplier_1, self.b[0])
        relu = nn.ReLU(graph, matrix_adder_1)
        matrix_multiplier_2 = nn.MatrixMultiply(graph, relu, self.W[1])
        matrix_adder_2 = nn.MatrixVectorAdd(graph, matrix_multiplier_2, self.b[1])
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(graph, matrix_adder_2, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(matrix_adder_2)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        negative_singleton = np.array([-1.0]).reshape(1,1)
        input_x_neg = nn.Input(graph, x.dot(negative_singleton))
        matrix_multiplier_3 = nn.MatrixMultiply(graph, input_x_neg, self.W[0])
        matrix_adder_3 = nn.MatrixVectorAdd(graph, matrix_multiplier_3, self.b[0])
        relu_2 = nn.ReLU(graph, matrix_adder_3)
        matrix_multiplier_4 = nn.MatrixMultiply(graph, relu_2, self.W[1])
        matrix_adder_4 = nn.MatrixVectorAdd(graph, matrix_multiplier_4, self.b[1])
        neg = nn.Input(graph, negative_singleton)
        matrix_multiplier_5 = nn.MatrixMultiply(graph, matrix_adder_4, neg)

        adder = nn.Add(graph, matrix_adder_2, matrix_multiplier_5)
        ...
        if y is not None:
            ...
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SquareLoss(graph, adder, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(adder)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.layer_num = 400
        self.W = [nn.Variable(784, self.layer_num), nn.Variable(self.layer_num, 10)]
        self.b = [nn.Variable(1, self.layer_num), nn.Variable(1, 10)]
        # self.W = [nn.Variable(784, self.layer_num), nn.Variable(self.layer_num, self.layer_num), nn.Variable(self.layer_num, 10)]
        # self.b = [nn.Variable(1, self.layer_num), nn.Variable(1, self.layer_num), nn.Variable(1, 10)]
    ...
    def run(self, x, y=None):
        ...
        "*** YOUR CODE HERE ***"
        graph = nn.Graph( [self.W[0], self.b[0], self.W[1], self.b[1]] )
        # graph = nn.Graph( [self.W[0], self.b[0], self.W[1], self.b[1], self.W[2], self.b[2]] )

        input_x = nn.Input(graph, x)
        matrix_multiplier_1 = nn.MatrixMultiply(graph, input_x, self.W[0])
        matrix_adder_1 = nn.MatrixVectorAdd(graph, matrix_multiplier_1, self.b[0])
        relu_1 = nn.ReLU(graph, matrix_adder_1)
        matrix_multiplier_2 = nn.MatrixMultiply(graph, relu_1, self.W[1])
        matrix_adder_2 = nn.MatrixVectorAdd(graph, matrix_multiplier_2, self.b[1])
        # relu_2 = nn.ReLU(graph, matrix_adder_2)
        # matrix_multiplier_3 = nn.MatrixMultiply(graph, relu_2, self.W[2])
        # matrix_adder_3 = nn.MatrixVectorAdd(graph, matrix_multiplier_3, self.b[2])

        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, matrix_adder_2, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(matrix_adder_2)
...
class DeepQModel(Model):
    ...
    def __init__(self):
        ...
        self.learning_rate = 0.01
        self.layer_num = 100
        self.W = [nn.Variable(self.state_size, self.layer_num), nn.Variable(self.layer_num, 2)]
        self.b = [nn.Variable(1, self.layer_num), nn.Variable(1, 2)]
    ...
    def run(self, states, Q_target=None):
        ...
        if Q_target is not None:
            "*** YOUR CODE HERE ***"
            input_Q = nn.Input(graph, Q_target)
            loss = nn.SquareLoss(graph, input_Q, matrix_adder_2)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(matrix_adder_2)
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.learning_rate = 0.01
        self.layer_num = 200
        self.W = [nn.Variable(self.num_chars, self.layer_num), nn.Variable(self.layer_num, len(self.languages))]
        self.Wh = [nn.Variable(self.layer_num, self.layer_num)]
        self.b = [nn.Variable(1, self.layer_num), nn.Variable(1, len(self.languages))]
    ...
    def run(self, xs, y=None):
        ...
        "*** YOUR CODE HERE ***"
        graph = nn.Graph([self.W[0], self.Wh[0], self.b[0], self.W[1], self.b[1]])
        H0 = nn.Input(graph, np.zeros([batch_size, self.layer_num]))

        for i in range(len(xs)):
            matrix_multiplier_0 = nn.MatrixMultiply(graph, H0, self.Wh[0])
            input_x = nn.Input(graph, xs[i])
            matrix_multiplier_1 = nn.MatrixMultiply(graph, input_x, self.W[0])
            adder = nn.Add(graph, matrix_multiplier_0, matrix_multiplier_1)
            matrix_adder = nn.MatrixVectorAdd(graph, adder, self.b[0])
            relu = nn.ReLU(graph, matrix_adder)
            H0 = relu

        matrix_multiplier = nn.MatrixMultiply(graph, relu, self.W[1])
        matrix_adder = nn.MatrixVectorAdd(graph, matrix_multiplier, self.b[1])

        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, matrix_adder, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output(matrix_adder)
