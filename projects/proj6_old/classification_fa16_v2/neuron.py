...
class SigmoidNeuron(object):
    def __init__(self, numFeatures):
        ...
        self.weight_vector = np.zeros(numFeatures, "float")
    ...
    def classify(self, sample):
        ...
        z = np.dot(self.weight_vector, sample)
        activation = np.exp(z) / (np.exp(z) + np.exp(-z))
    ...
    def train_single(self, sample, alpha, label):
        ...
        def helper(w):
            return SigmoidNeuron.loss(self, sample, label, w)

        if not self.classify(sample):
            self.weight_vector -= alpha * nu.gradient(helper, self.weight_vector)
